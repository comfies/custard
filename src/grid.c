...
unsigned int grid_get_span(unsigned int unit_size, unsigned int amount) {
    return (unit_size * amount) + (grid_gap * (amount - 1)) + (
        (border_total_size * 2) * (amount - 1));
}

unsigned int grid_get_offset(unsigned int unit_size, unsigned int amount) {
    return (grid_gap * (amount + 1)) + (unit_size * amount) + (
        (border_total_size * 2) * amount);
}

unsigned int grid_get_span_x(unsigned int amount) {
    return grid_get_span(grid_x_unit_size_in_pixels, amount);
}

unsigned int grid_get_span_y(unsigned int amount) {
    return grid_get_span(grid_y_unit_size_in_pixels, amount);
}

unsigned int grid_get_x_offset(unsigned int amount) {
    return grid_get_offset(grid_x_unit_size_in_pixels, amount);
}

unsigned int grid_get_y_offset(unsigned int amount) {
